...
- Phase 3 Tier 1 progress:
  ...
  - `AL_001` was further upgraded to AST-driven table-factor alias traversal with token-span-aware alias-context checks (shared document tokens first, tokenizer fallback) for explicit vs implicit `AS` detection.
  ...
  - `AL_002` was further upgraded to AST-driven SELECT projection alias traversal with token-span-aware alias-context checks (shared document tokens first, tokenizer fallback) for explicit vs implicit `AS` detection.
  ...
  - `AL_002` alias-style and TSQL assignment classification now run on token-span context only (shared document tokens with tokenizer fallback), removing raw text-parsing fallback paths.
  ...
  - `ST_003` unused-CTE issue spans now derive from AST identifier spans (`cte.alias.name.span`) only (no SQL-text search fallback), removing span heuristics from CTE-definition highlighting.
...
|---|---|---|
| AL_001 (aliasing.table) | Core (partial) | Remaining advanced SQLFluff edge cases beyond current AST + token-span explicit/implicit alias checks |
| AL_002 (aliasing.column) | Core (partial) | Remaining advanced SQLFluff edge cases beyond current AST + token-span explicit/implicit alias checks |
