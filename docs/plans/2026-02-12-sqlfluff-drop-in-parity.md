...
- Phase 3 Tier 1 progress:
  ...
  - `ST_010` now aligns closer with SQLFluff ST10 comparison semantics by detecting equivalent-expression predicate comparisons across `=`/`!=`/`<`/`>`/`<=`/`>=` (e.g. `x = x`, `x < x`) with operator-side guardrails while preserving SQLFluff-style literal handling (`1=1`/`1=0` allow-list and non-equality literal-vs-literal deferral).
