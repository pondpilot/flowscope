...
- [x] `LINT_LT_001`/`LINT_LT_005`/`LINT_LT_006`/`LINT_LT_008`/`LINT_LT_009`/`LINT_LT_014` are now split out of `parity.rs` into dedicated core modules (`lt_001.rs`, `lt_005.rs`, `lt_006.rs`, `lt_008.rs`, `lt_009.rs`, `lt_014.rs`); `LINT_LT_006` now uses token-stream spacing detection for function-like calls, `LINT_LT_009` now uses tokenizer-located SELECT-line target counting, and `LINT_LT_014` now uses token/line-aware major-clause placement checks instead of regex masking.
