...
- [x] `LINT_ST_012` and `LINT_TQ_001`-`LINT_TQ_003` are now split out of `parity.rs` into dedicated core modules (`st_012.rs`, `tq_001.rs`-`tq_003.rs`); `LINT_TQ_001`/`LINT_TQ_002` are now AST-driven (`CreateProcedure` name/body analysis), while `LINT_ST_012`/`LINT_TQ_003` currently preserve heuristic behavior.
- [x] `LINT_CV_001`, `LINT_CV_007`, and `LINT_CV_009`-`LINT_CV_011` are now split out of `parity.rs` into dedicated core modules (`cv_001.rs`, `cv_007.rs`, `cv_009.rs`-`cv_011.rs`); `LINT_CV_007`, `LINT_CV_009`, `LINT_CV_010`, and `LINT_CV_011` are now AST-driven, while `LINT_CV_001` currently preserves heuristic behavior.
