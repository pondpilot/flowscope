...
- [x] `LINT_RF_004`/`LINT_RF_005`/`LINT_RF_006` are now split out of `parity.rs` into dedicated core modules (`rf_004.rs`-`rf_006.rs`); all three now use AST-driven traversal (`RF04` identifier/alias analysis, `RF05`/`RF06` shared quoted-identifier traversal).
