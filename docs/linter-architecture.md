...
- [x] `LINT_RF_004`/`LINT_RF_005`/`LINT_RF_006` are now split out of `parity.rs` into dedicated core modules (`rf_004.rs`-`rf_006.rs`); `LINT_RF_004` has been upgraded to AST-driven alias analysis while `LINT_RF_005`/`LINT_RF_006` remain heuristic.
...
- [x] `LINT_CV_001`, `LINT_CV_007`, and `LINT_CV_009`-`LINT_CV_011` are now split out of `parity.rs` into dedicated core modules (`cv_001.rs`, `cv_007.rs`, `cv_009.rs`-`cv_011.rs`); `LINT_CV_009` has been upgraded to AST-driven identifier/table/alias traversal while remaining rules currently preserve heuristic behavior.
