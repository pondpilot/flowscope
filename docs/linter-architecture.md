...
- [x] `LINT_ST_004` now includes SQLFluff ST07 fixer parity via AST join-constraint rewrites, converting `JOIN ... USING (...)` to explicit `ON` predicates (including multi-column USING lists).
...
- [~] SQLFluff fixture adoption is in progress for migrated rules; AM02/AM03/AM04/AM05/AM06/AM07/AM08/AM09, CV02, CV05, ST04, ST06, ST07, and ST09 fixture cases were adopted for `LINT_AM_001`/`LINT_AM_005`/`LINT_AM_004`/`LINT_AM_006`/`LINT_AM_007`/`LINT_AM_008`/`LINT_AM_009`/`LINT_AM_002`/`LINT_CV_001`/`LINT_CV_003`/`LINT_ST_003`/`LINT_ST_004`/`LINT_ST_007`/`LINT_ST_009`, and additional rule-level coverage is still being expanded.
