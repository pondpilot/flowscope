...
| Ambiguous | `ambiguous.union` | `AM02` | Yes | Yes | Implemented (close) | `LINT_AM_001` | Yes | AST rule in `crates/flowscope-core/src/linter/rules/am_001.rs` matches SQLFluff AM02 bare-`UNION` detection semantics, fixer inserts explicit `DISTINCT` via AST set-quantifier rewrite (SQLFluff-style), and rule execution is now dialect-scoped to SQLFluff-supported dialects available in FlowScope. |
| Ambiguous | `ambiguous.order_by` | `AM03` | No | Yes | Implemented (close) | `LINT_AM_005` | Yes | AST rule in `crates/flowscope-core/src/linter/rules/am_005.rs` matches SQLFluff AM03 mixed-direction detection semantics; fixer now normalizes mixed clauses by adding explicit `ASC` to implicit items (including `NULLS` ordering cases). |
...
| Structure | `structure.simple_case` | `ST02` | No | Yes | Implemented (partial) | `LINT_ST_005` | Yes | Heuristic parity rule implemented in `crates/flowscope-core/src/linter/rules/parity.rs`; fixer now uses AST CASE rewrites for SQLFluff-style conversion from repeated `WHEN x = ...` patterns to simple CASE form. |
...
| Structure | `structure.column_order` | `ST06` | No | Yes | Implemented (partial) | `LINT_ST_007` | Yes | Heuristic parity rule implemented in `crates/flowscope-core/src/linter/rules/parity.rs`; fixer now uses AST projection reordering for SQLFluff-style simple-before-complex column ordering in SELECT targets. |
