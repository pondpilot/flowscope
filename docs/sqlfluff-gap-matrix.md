...
| Ambiguous | `ambiguous.set_columns` | `AM07` | No | No | Implemented (partial) | `LINT_AM_008` | No | AST rule implemented in `crates/flowscope-core/src/linter/rules/am_008.rs`; currently narrower than SQLFluff AM07 (focuses on wildcard usage in set branches). |
