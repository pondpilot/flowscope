...
| Structure | `structure.simple_case` | `ST02` | No | Yes | Implemented (partial) | `LINT_ST_002` | Yes | Heuristic parity rule implemented in `crates/flowscope-core/src/linter/rules/parity.rs`; fixer now uses AST CASE rewrites for SQLFluff-style conversion from repeated `WHEN x = ...` patterns to simple CASE form. |
...
| Structure | `structure.column_order` | `ST06` | No | Yes | Implemented (partial) | `LINT_ST_006` | Yes | Heuristic parity rule implemented in `crates/flowscope-core/src/linter/rules/parity.rs`; fixer now uses AST projection reordering for SQLFluff-style simple-before-complex column ordering in SELECT targets. |
...
| Structure | `structure.distinct` | `ST08` | Yes | Yes | Implemented (partial) | `LINT_ST_008` | Yes | Heuristic parity rule implemented in `crates/flowscope-core/src/linter/rules/parity.rs`; fixer now uses AST projection rewrite to normalize `SELECT DISTINCT(a)` into SQLFluff-style `SELECT DISTINCT a`. |
